...
import org.perfcake.idea.oldmodel.MessageModel;
import org.perfcake.idea.oldmodel.MessagesModel;
...
public class MessagesRectangle extends JTitledRoundedRectangle implements PropertyChangeListener {
    ...
    private void updateRectangle() {
        //oldmodel changed, first remove all messages
        synchronized (getTreeLock()) {
            Component[] components = panel.getComponents();
            for (Component c : components) {
                if (c instanceof MessageRectangle) {
        ...
        //add messages from updated oldmodel
        addMessages();
    }
}
